...
function Logo(props: { src: string, alt: string, enlarge?: boolean, height: number, width?: number }) {
    if (props.enlarge) {
        return <Image src={props.src} height={props.height} width={props.width??280} alt={props.alt} layout="intrinsic" className=" shrink-0 max-h-20 opacity-40 hover:opacity-100 hover:brightness-100 brightness-0 contrast-100 cursor-pointer" />
    } else {
        return <Image src={props.src} height={props.height} width={props.width??200} alt={props.alt} layout="intrinsic" className=" shrink-0 max-h-15 opacity-40 hover:opacity-100 hover:brightness-100 brightness-0 contrast-100 cursor-pointer" />
    }
}

export default function CollaborationSection() {
    return <div className="flex flex-col items-center sm:items-start gap-4 sm:mx-10 mt-10">
        <div>
        ...
        <div className="fade-out-container overflow-hidden max-w-full">
            <div className=" flex flex-row gap-16 overflow-hidden items-center w-max autoscroll shrink-1 grow">
                ...
                <Logo src="/logos/nordicsemi.svg" alt="Nordic semiconductor Logo - Trademark of nordic semiconductor" height={53.13}/>
